...
export const OrderUpdateCard = ({ order, user, reloadData }) => {
  ...
  const orderStatusUpdateHandler = async (values) => {
    await updateOrderStatusData(order._id, {
  ...
  const paymentStatusUpdateHandler = async (values) => {
    await updatePaymentStatusData(order._id, {
      payment_status: values.is_paid ? 'SUCCESS' : 'PENDING',
    });
    reloadData(order._id);
  ...
  return (
    <div className='card'>
      ...
      <div className='card-body'>
        {editing && user && (user.role === 'SUPER ADMIN' || user.role === 'VENDOR') ? (
          <div>
            <Formik
              initialValues={{
          ...
          <div className='d-flex justify-content-between'>
            ...
            {user && (user.role === 'SUPER ADMIN' || user.role === 'VENDOR') && (
              <div>
                {' '}
                <button
